...
export default function PythonExercisePage() {
  ...
  if (!exercise || exercise.topicId !== 'python') {
    return (
  ...
  const exerciseIndex = pythonExercises.findIndex((e) => e.id === exercise.id);
  const previousExercise = exerciseIndex > 0 ? pythonExercises[exerciseIndex - 1] : null;
  const nextExercise = exerciseIndex < pythonExercises.length - 1 ? pythonExercises[exerciseIndex + 1] : null;
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="flex gap-4 justify-between mt-12 pt-8 border-t border-slate-200 dark:border-slate-800">
          {previousExercise ? (
            <Link href={`/temas/python/ejercicios/${previousExercise.slug}`}>
          ...
          {nextExercise ? (
            <Link href={`/temas/python/ejercicios/${nextExercise.slug}`}>
              <Button>
                Siguiente Ejercicio →
              </Button>
            </Link>
